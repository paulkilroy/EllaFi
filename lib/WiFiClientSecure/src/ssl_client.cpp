...
void stop_ssl_socket(sslclient_context *ssl_client, const char *rootCABuff, const char *cli_cert, const char *cli_key)
{
    ...
    // EllaFi PATCH: the memset above clobbered `socket` (field at offset 0) to 0. Upstream forgets to
    // restore it, so after a failed connect (which calls stop()) the fd reads 0 — and WiFiClientSecure::
    // stop()'s `if (socket >= 0)` then close(0)s on the destructor's second stop(). close(0) frees stdin,
    // LittleFS recycles fd 0, and the next close(0) corrupts LittleFS (lfs_mlist_isopen assert). Restore -1.
    ssl_client->socket = -1;
}
